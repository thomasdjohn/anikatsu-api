...
export const scrapeAnimeDetails = async ({ id }) => {
  try {
    ...
    const otherName = $('div.anime_info_body_bg > p.other-name')
        .text()
        .replace('Other name: ', '') // Loại bỏ đoạn "Other name: "
        .replace(/;/g, ',')          // Thay dấu chấm phẩy (;) bằng dấu phẩy (,)
        .replace(/\s+/g, ' ')        // Loại bỏ nhiều khoảng trắng hoặc xuống dòng thành một khoảng trắng
